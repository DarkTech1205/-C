...
- **New failure, unrelated to anything above:** `object-collab` auto-
  updated to `v0.6.0` (our `mod.json` only pinned a floor,
  `>=0.5.5-beta.3`) and that version added a second third-party
  dependency, `alphalaneous.editortab_api`, that was never declared —
  so it never got fetched, breaking every file that includes
  `object_collab.hpp`. Added it to `mod.json`'s dependencies.
  **Honesty check:** I don't know its real minimum version — `>=1.0.0` is
  a reasonable floor guess, not a confirmed number. If the dependency
  resolver rejects it as "not found" or "no matching version," that's
  the next thing to fix, and the fix is just adjusting this one version
